...
scipy.misc.imsave('enhance.png',enhanced_img)
...
for i in coeffs:
        ...
        #New = i[0,:,:]
        #print New.shape
        New = i.astype(np.uint8)
        #New = cv2.normalize(New, None)
        histf = cv2.calcHist([New],[0],None,[256],[0,350])
        histf = cv2.normalize(histf, None)
        output_list.append(histf)
        plt.plot(histf,'r')
        plt.show()
                      
for i in output_list:
        for a in range(1,7):
                #axes[a].set_title('Wave decomp')
                #axes[a].plot(i,'r')
plt.show()

for i in wavelet_dec:
    for a in range(1,7):
        axes[a].set_xlim([0, 350])
        axes[a].set_title('Wave decomp')
        axes[a].plot(i,'r')
plt.show()
